...
export default function Home() {
  ...
  const sendHome = async () => {
    try {
      // Encode the '$' character in the command
      const cmd = encodeURIComponent('$h');
      const url = `http://amazing.local/command?cmd=${cmd}`;
      ...
      if (response.ok) {
        ...
        console.error('Failed to send POST request:', await response.text());
        // Handle error response if needed
      }
    } catch (error) {
      console.error('Error in sendHome:', error);
      // Handle network errors if needed
    }
  };
  const sendPrime = async () => {
    try {
      // Encode the '$' character in the command
      const cmd = encodeURIComponent('g69”setret0a1000b1000c1000d1000e1000f0”\n$SD/Run=prime.g');
  ...
  const sendPrint = async () => {
    try {
      // Encode the '$' character in the command
      const cmd = encodeURIComponent('$SD/Run=colors.g');
  ...
  const sendToPalette = async () => {
    try {
      const swatchInputs = colors.map((c) =>
        convertColorEntryToSwatchInput(c, svgContents, 0)
      );
      ...
      // Update the endpoint to send to "http://amazing.local/sdfiles/colors.txt"
      const endpoint = "http://amazing.local/upload";
      ...
      formData.append("upload", blob, "colors.g");
